...
| suíte | 172 arquivos / 2.975 testes (vitest) + 589 testes e 124 subtests (pytest, lab). Verde em rodada limpa e SOZINHA; com uma segunda corrida de vitest concorrente, dois a quatro arquivos de caminho selado batem no timeout de 20 s — dívida de § 7, não de política |
...
| `evaluatorDigest` da árvore | `fdb42887ade3715d10470c778282a76e5bc44e6fb0210b13dd5635b863f551fb` — 52 arquivos, recomputado pela função de produção e **lido por teste nomeado** (`digests.test.ts`, "is published in the ESTADO at the value the LIVE tree hashes to"), então este número não pode envelhecer em silêncio. Mover é barato enquanto `issuedAt` é nulo |
...
| dívida | vence |
|---|---|
| a **fila do cross-review de dez unidades**: R1 pagou o `consolidado-c`, R2 o `consolidado-w1`, e as ondas **A1** e **A2** (2026-08-11) pagaram **sete** dos nove que mordiam agora — `validate` recusando antes de selar, o eixo do ledger por totalidade, o manifesto com guarda de localizador, o piso barato pareado por fatia, o `extractionRun` carimbado pelo extrator, a pertença de célula exigida no selo e o recibo do gate dentro do artefato selado. **Sobram 2 dos 9 que mordem agora** (as linhas abaixo), **14 que mordem antes de publicar** e **31 menores** ainda não refutados um a um. A auditoria e o que foi refutado estão no registro (§ "A auditoria de 2026-08-10"); a onda A1, com as duas rodadas de revisão que ela precisou, em § "A onda A1" | as cinco abaixo, agora; as catorze, antes da Fase 6 |

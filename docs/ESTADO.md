...
| o **eixo de operação de primeira classe não existe**, e a dívida tem duas metades com prazos diferentes. A PRIMEIRA é a `proxyReason` selada de `resampling.estimandClasses.mixed.levels[1]`, que tem duas orações: "nenhum eixo do schema v4 registra a operação" continua **verdadeira** — `mixOperation` é campo da linha de POOL e `mixed_record` não o carrega para o registro selado, cujo `MIXTURE_KEYS` e `GROUPS_KEYS` são fechados —, mas "`groups.promptTemplate` tem um único nível sobre as linhas mistas montadas" fica **falsa** na primeira montagem sob este desenho: são três níveis por ilha, sessenta no corpus, e é exactamente o que faz a reamostragem aninhada `humanSeed × promptTemplate` isolar a operação. Emendá-la move o `evaluatorDigest` e obriga a repinar `SEALED_POLICY_SHA256` no mesmo commit, então é unidade própria — e vence **antes** de existir linha mista montada, não em v2. A SEGUNDA é o eixo lido em vez de inferido: o eixo carrega o **digest do template**, não a operação; o plano já torna irrepresentáveis duas identidades da mesma operação numa ilha (`mixingTemplates` é mapa chaveado pela operação), mas nada no ESQUEMA o recusa | a `proxyReason`, antes de gerar a classe mista; o eixo, v2 — é mudança de `V4_GROUP_AXES` e move o `evaluatorDigest` |

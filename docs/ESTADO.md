...
| suíte | 172 arquivos / 3.127 testes (vitest) + 774 testes e 970 subtests (pytest, lab). Verde em rodada limpa e SOZINHA; com uma segunda corrida de vitest concorrente, dois a quatro arquivos de caminho selado batem no timeout de 20 s — dívida de § 7, não de política |
...
1. **implementar o `llm-pii-screen`** — minha, unidade própria com TDD e bateria, e vence antes
   da montagem da Fase 3. **A METADE DE CONTRATO ESTÁ FEITA (2026-08-24):** `llm-pii-screen` é membro
   de `AUTOMATED_FILTERS`, o perfil `census-pii-screen-v1` está **pré-inscrito** em
   `benchmark/assurance-profile.ts` com as três recusas do selo, e o `evaluatorDigest` moveu a **uma**
   vez que estava orçada (52 → 53 arquivos) — o custo medido está em § 5.14. **O que falta é a metade
   do lab:** a triagem no funil de `assemble_corpus.main()`, o ledger de disposições, os controles com
   `S_control` por estrato, os gates adversariais por pares e o recibo `pii-screening-receipt.json`;
...
intervalo também. O que fica na § 7 para antes da primeira corrida **certificadora** é o gate que
confunde tentativas pedidas com réplicas finitas, e ele não está no caminho da geração.
...
| **`enforce_unique_keys` renomeia o PAI, e a ponte pai↔mista quebra em silêncio.** Medido com fixture nas duas direções: os pools são entregues `(ai, mixed, humans)`, então quando `mixed.parentId` colide com `human.candidateId` quem é renomeado é o **humano** — a mista guarda o valor com que nomeia o pai e a chave do pai move-se debaixo dela. `connected_components` une por `derivationRoot` com a condição `named in ids`, que exige um **id de registro presente**, então a união é saltada **em silêncio**: sem renomeio o par mede **1** componente, com renomeio mede **2**. A ponte que a § 5.4c e a § 5.12 declaram é a que se perde. **A condição estreita o achado:** o par só colide se os dois sobreviverem à poda — nas células de Jaccard ≥ 0,82 (0,848–0,869 em `insercao/25`, § U.3 de `references.md`) o pai já cai e a ponte já não existe, que é o achado antigo; o achado NOVO é sobre as células abaixo de 0,82 (a segunda mede 0,774), onde os dois sobrevivem. **O diagnóstico é de tipo:** `parentId` é chave estrangeira, e passá-la a um impositor de unicidade que a mutila é tratar referência como identidade — reordenar os pools inverteria quem é renomeado e quebraria a ponte do outro lado. O conserto é chavear a mista pelo id dela (`mix_<parent>`), e isso muda quais linhas são renomeadas numa montagem real, logo muda contagens de componente que o plano ratificado da ilha usa: **unidade própria, com as três etapas** | antes da montagem da Fase 3, e antes de gastar a cota de geração — a matriz de cobertura é declarada contra a conectividade da ilha |

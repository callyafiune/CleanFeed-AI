...
| suíte | 174 arquivos / 3.162 testes (vitest) + 982 testes e 970 subtests (pytest, lab). Verde em rodada limpa e SOZINHA; com uma segunda corrida de vitest concorrente, dois a quatro arquivos de caminho selado batem no timeout de 20 s — dívida de § 7, não de política |
...
| | **a montagem abre os pools que lhe DÃO**: `load_humans(cand, reserved, pools)` percorre `pools`, cuja default é `HUMAN_POOL_FILES` — e `wikipedia_fresh`, a extração de 06/08, fica **fora** dela porque as 4.100 linhas sem carimbo rendem zero registro (§ 5.15). `--human-pool` é repetível na CLI. Selecção com pools que partilhem `candidateId` é **recusada** (`OverlappingHumanPools`) e não deduplicada: `dedup` colapsa por hash de texto e guarda a PRIMEIRA ocorrência, então com os dois pools reais — texto idêntico nos 4.100 ids, só o carimbo a diferir — seria a **ordem da tupla** a decidir entre 4.100 registros e zero, sem nada visível a mudar | código |
...
| **PAGA em 2026-08-25 — a montagem lia um nome constante, e o nome decidia a classe humana inteira.** `load_humans` percorria `("wikipedia_fresh",)`, então abria o pool de 06/08 e `human_record` recusava as 4.100 linhas dele: a classe humana da montagem era **zero** com o pool carimbado em disco ao lado. A lista passou a ser o terceiro parâmetro, com `--human-pool` repetível na CLI, e a mesma corrida sobre o material real mede as três pontas: default colhe 4.100 e constrói **4.100**; `pools=("wikipedia_fresh",)` colhe 4.100 e constrói **0**, todas `MissingExtractionRun`; os dois juntos são **recusados** nomeando o `candidateId` e os dois arquivos. **O que NÃO entrou, e a razão é medida:** `diagnostic_probes.IN_FRAME_POOLS` e `ner_pilot.DEFAULT_POOLS` já são default de argumento (`--pool-file`, `--pools`), e `IN_FRAME_POOLS` é a receita que reproduz as taxas publicadas em § 5.7 sobre `benchmark/data/candidates` — movê-la moveria a população de um número publicado. Bateria de **quatro** mutações, quatro mortas, restauração conferida por sha256 | fechada |

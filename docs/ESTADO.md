...
| suíte | 172 arquivos / 3.115 testes (vitest) + 741 testes e 742 subtests (pytest, lab). Verde em rodada limpa e SOZINHA; com uma segunda corrida de vitest concorrente, dois a quatro arquivos de caminho selado batem no timeout de 20 s — dívida de § 7, não de política |
| dos quais, o avaliador | 2.479 — 1.738 em 46 arquivos de `benchmark/tests`, 741 no lab |
| typecheck | limpo |
...
| dívida | vence |
|---|---|
| as 400 linhas da reserva em disco são **escrevíveis** e o slate as reserva: `ollama` é lane congelada, o provedor mapeia, e `qwen2_5-7b-q4km` tem o papel `ood-reserved`. Medido: 400 de 400 produzem registro v3 e a montagem declara a reserva. **O que fica aberto é material, e são duas faltas.** (i) A **segunda linhagem ratificada não está no slate**, e a ausência é medida e não escolha: a família do llama não tem linha em disco, e `assert_slate_roles_are_consistent` recusa papel sobre família que os pools não entregam — cobertura de nada é o defeito que essa guarda existe para pegar. A direção é fail-fechada: no dia em que as linhas existirem sem papel, a montagem **para** em `UndeclaredGeneratorFamily`. (ii) A **pista de geração do ollama não existe no repositório** — medido, nenhum arquivo menciona `ai_reserved_qwen` como escritor, `ollama` ou a porta 11434 —, então as 50 que faltam até 450, as 450 do llama e qualquer regeração exigem escrevê-la. **E o template daquelas 400 é irrecuperável:** o `promptSha256` é `f823530d215fffe8…` e os bytes que o produzem não estão em lugar nenhum (procurado no repositório e no snapshot; quinze reconstruções plausíveis testadas, nenhuma casa). Logo a família fica com **duas identidades de prompt** (400 sob o digest perdido, o resto sob o novo) a menos que o operador mande regerar as 450 — e regerar é apagar material | (i) quando existir linha do llama; (ii) a pista, unidade própria; a escolha das duas identidades de prompt é **do operador** |

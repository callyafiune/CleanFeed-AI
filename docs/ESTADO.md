...
| a **guarda pré-cota da classe MISTA não existe**: os três `make_mixed*.py` usam um template único de mistura — todos declaram o mesmo `mix_edit_v1` —, e medido isso leva o corpo de 20 componentes a **um de 100 %**; pais mistos espalhados, também. A FORMA do plano hospeda o desenho — `_island()` declara os três slots por operação, medido —, e o que falta é a pista escrever a identidade por ilha × operação em vez de um nome só. E a banda legada `MIXED_BAND (0,05–0,7)` **recusaria v6 e v7**: a pista nova não pode herdá-la. A cota mista ainda não foi gasta, então a janela está aberta | unidade seguinte, antes de gerar a classe mista |

...
| suíte | 172 arquivos / 3.046 testes (vitest) + 668 testes e 437 subtests (pytest, lab). Verde em rodada limpa e SOZINHA; com uma segunda corrida de vitest concorrente, dois a quatro arquivos de caminho selado batem no timeout de 20 s — dívida de § 7, não de política |
...
| re-rodar ou não o codex nas unidades do caminho selado que carregam **só** revisão do Fable. A trava antiga ("no retorno do crédito") **já disparou** e o conjunto encolheu de dez para **duas**, nomeadas aqui porque a decisão de origem prometeu visibilidade e os únicos artefatos que as nomeavam vivem em `.codex-reviews/`, que o `.gitignore` cobre: **Fase 1 Commit E** — o gate de composição (`benchmark/composition-gate.ts`, commit `7c8b782`) — e **Fase 1 Commit F** — o preflight de viabilidade (`benchmark/viability-preflight.ts`, commit `8511eb1`). As duas foram as posições 9 e 10 da fila de prioridade, e o corte por cota caiu exatamente nelas: codex `EXIT=1` com zero token, revisão do Fable na mesma noite, e as duas se declaram devedoras de codex no próprio cabeçalho. Mais **R1** e **R2** (2026-08-10), que nasceram com a janela já fechada | janela de cota aberta (a próxima imprime 16 de agosto) **e** decisão do operador de gastar nelas em vez de em outra rodada |
...
| `evaluatorDigest` da árvore | `0d278f872b829d413b0aa69e63ede695621e3a39e6612ed37919e9e68357295c` — 52 arquivos, recomputado pela função de produção e **lido por teste nomeado** (`digests.test.ts`, "is published in the ESTADO at the value the LIVE tree hashes to"), então este número não pode envelhecer em silêncio. Mover é barato enquanto `issuedAt` é nulo |
...
| dívida | vence |
|---|---|
| **o slate não cumpre o plano de ilhas, e por isso nenhuma linha pode ser gerada**: `island_plan` recusa TODA ilha — `RECIPES` declara **4** identidades de template. O ramo deixou de ser aberto: o desenho de 2026-08-12 (§ 3.3) fixa o preço em **100 identidades** — 20 ilhas × (2 de geração + 3 de mistura) —, então o que o operador ratifica é um número e não uma escolha entre crescer o slate e emendar o plano. Encolher a cota **não** reduz o número de ilhas: o piso é sobre fração (`1/n ≤ 0,05 + 0,02`), logo é livre de escala. É a guarda funcionando antes da cota, não defeito | **decisão do operador**, antes da Fase 3 item 2 |
| a **guarda pré-cota da classe MISTA não existe**: os três `make_mixed*.py` usam um template único de mistura — todos declaram o mesmo `mix_edit_v1` —, e medido isso leva o corpo de 20 componentes a **um de 100 %**; pais mistos espalhados, também. O plano hoje declara **um** slot `mixingTemplate` por ilha e o desenho decidido pede **três**, um por operação, então a forma do plano também não hospeda o desenho. E a banda legada `MIXED_BAND (0,05–0,7)` **recusaria v6 e v7**: a pista nova não pode herdá-la. A cota mista ainda não foi gasta, então a janela está aberta | unidade seguinte, antes de gerar a classe mista |
| **nada impõe o piso 0,50 do misto na v1**: o gate é diagnóstico por emenda (§ 3.1), o número sai no relatório e no perfil, e a barreira contra um model card que omita a limitação é **prosa mais teste de presença**, não mecanismo. O recall esperado é da ordem de 11–20 % pela literatura e pelo corpus morto, logo a v1 pode sair avisando em **menos da metade** dos textos que a própria política define como positivos de aviso. Rearmar exige formulação nova **e** piso com fonte, e dimensionamento próprio por D0b sobre a coorte de 240 do bloco cego | v2, ou a unidade que trocar a formulação; a declaração da limitação vence na Fase 6 |
| o **eixo de operação de primeira classe não existe**, e o desenho de 2026-08-12 muda o que falta: com três templates de mistura por ilha, `groups.promptTemplate` passa a ter três níveis por ilha e a reamostragem aninhada `humanSeed × promptTemplate` isola a operação — então a `proxyReason` selada, que declara o fator "degenerado por construção até um eixo de operação existir", fica **falsa** e tem de ser emendada no mesmo commit. O que resta de dívida é o eixo lido em vez de inferido: o eixo carrega o **digest do template**, não a operação, então duas identidades da mesma operação numa ilha são indistinguíveis de duas operações e nada no esquema recusa esse plano | v2 — é mudança de `V4_GROUP_AXES` e move o `evaluatorDigest` |
| a matriz **nível × operação** está fixada por **sonda e não por teste**: os Jaccard de 0,848–0,869 de v1 × inserção e o 0,774 da célula seguinte foram medidos com as funções de produção de `near_dupes.py` sobre pais sintéticos de 100 a 1.200 tokens, com tokens todos distintos — suposição declarada, porque texto real repete token. Enquanto a pista não prender os números por fixture, "célula inalcançável" é memória e não medição | unidade que escrever `make_mixed_v3.py` |
...
| rodada 13 do cross-review do E2: **nunca rodou**, e a trava mudou de natureza. O crédito que ela esperava **voltou** na janela prometida (8 de agosto) e foi **gasto em outra coisa** — a rodada das dez unidades de 9 de agosto —, então o que a bloqueia hoje não é ausência de crédito, é **alocação**: a próxima janela é disputada entre ela, as duas unidades de § 4 e o que vier. As doze rodadas anteriores têm veredito (`e2-r1` a `e2-r12`); a décima terceira tem prompt, runner e o arquivo da tentativa que morreu por cota em 2026-08-02 | próxima janela de cota **e** a decisão do operador sobre em que gastá-la |

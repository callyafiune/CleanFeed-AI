...
| suíte | 172 arquivos / 3.115 testes (vitest) + 742 testes e 742 subtests (pytest, lab). Verde em rodada limpa e SOZINHA; com uma segunda corrida de vitest concorrente, dois a quatro arquivos de caminho selado batem no timeout de 20 s — dívida de § 7, não de política |
| dos quais, o avaliador | 2.480 — 1.738 em 46 arquivos de `benchmark/tests`, 742 no lab |
| typecheck | limpo |
...
| | `--provider` recusa na **argparse**, com as quatro lanes que `generate_ai.py` dirige como `choices` — subconjunto das seis congeladas, e o complemento (`claude-code`, `ollama`) é conferido por nome: nenhuma chamada de provedor fora do slate é gasta | AG |
...
| dívida | vence |
|---|---|
| as 400 linhas da reserva em disco são **escrevíveis** e o slate as reserva: `ollama` é lane congelada, o provedor mapeia, e `qwen2_5-7b-q4km` tem o papel `ood-reserved`. Medido: 400 de 400 produzem registro v3 e a montagem declara a reserva. **O que fica aberto é material, e são duas faltas.** (i) A segunda linhagem ratificada tem **nome no código e nenhum papel**: ela vive em `RATIFIED_PENDING_RESERVE`, não em `OOD_RESERVED_FAMILIES`, porque papel é alegação sobre material e a família do llama não tem linha em disco. O nome é DERIVADO por `reserve_family(tag, quantização)`, a mesma função que a pista de geração vai usar, e a âncora que a torna confiável é ela reproduzir a família que as 400 linhas já carregam. Três guardas, e o que elas compram não é tempo: as duas regras de cobertura já recusavam os dois primeiros estados na MESMA chamada, no topo de `main()` e antes de qualquer pool abrir. O ganho é a mensagem **específica** no lugar da genérica — "declare-a `ood-reserved`, foi o que ela foi ratificada como" contra "família sem papel" —, e por isso as pendentes correm antes. A terceira é a única que pega estado que nenhuma outra regra olha: a **grafia** da lista pendente, que nada mais lê, e o dia da promoção é exactamente quando ninguém a relê. (ii) A **pista de geração do ollama não existe no repositório** — medido, nenhum arquivo menciona `ai_reserved_qwen` como escritor, `ollama` ou a porta 11434 —, então as 50 que faltam até 450, as 450 do llama e qualquer regeração exigem escrevê-la. **E o template daquelas 400 NÃO FOI RECUPERADO:** o `promptSha256` é `f823530d215fffe8…`, os bytes não estão no repositório nem no snapshot fora dele, e quinze reconstruções plausíveis testadas contra o digesto não casam. Isso não prova que sejam irrecuperáveis — um acerto seria prova, uma falha não é —, e o que a busca estabelece é que nenhum caminho conhecido os devolve. Logo a família fica com **duas identidades de prompt** (400 sob o digest perdido, o resto sob o novo) a menos que o operador mande regerar as 450 — e regerar é apagar material | (i) quando existir linha do llama; (ii) a pista, unidade própria; a escolha das duas identidades de prompt é **do operador** |

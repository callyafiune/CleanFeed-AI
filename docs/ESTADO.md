...
| suíte | 172 arquivos / 3.115 testes (vitest) + 756 testes e 750 subtests (pytest, lab). Verde em rodada limpa e SOZINHA; com uma segunda corrida de vitest concorrente, dois a quatro arquivos de caminho selado batem no timeout de 20 s — dívida de § 7, não de política |
| dos quais, o avaliador | 2.494 — 1.738 em 46 arquivos de `benchmark/tests`, 756 no lab |
| typecheck | limpo |
...
| **nada cruza modelo e effort com template, e a lacuna é de PLANO.** Medido: uma ilha carrega `templates`, `mixingTemplates`, `seedBlock`, `lines` e `reserved`; uma receita carrega `task`, `register`, `template` e `weight`. **Nenhum dos dois nomeia modelo ou effort** — os dois são argumentos POR CORRIDA. A identidade de PROMPT é planejada e particionada por ilha (40 templates, `promptTemplate` na união do split); a de GERAÇÃO — 24 no codex, 12 no agy, 20 no Claude Code — é o que a linha de comando fizer, e nada garante que um modelo apareça em mais de uma ilha. **DUAS CONSEQUÊNCIAS FORAM PROCURADAS EM 2026-08-22 E NENHUMA DAS DUAS SE SUSTENTA.** (i) A ameaça de ATRIBUIÇÃO por fatia **não existe**: nenhuma fatia é chaveada por família, modelo ou effort — `SliceAxis` tem dez membros e o único que toca gerador, `generatorExposure`, tem dois valores, `seen` e `unseen`. Não há número publicado que a colinearidade modelo↔template possa confundir. (ii) A ameaça ao INTERVALO **foi medida e refutada**: a unidade de `ai-recall` aninha `promptTemplate` dentro de `generatorFamily`, e eu supus que uma família num só cluster de template estreitaria o intervalo. Medido pela função de produção com a variabilidade intra-cluster FIXA e só o rótulo de template a variar, nos dois regimes de lote (uma unidade por linha e lotes de cinco): a largura fica **plana** — 0,2167 com um template e 0,2167 com dois no primeiro regime, 0,2250 e 0,2000 no segundo, sem tendência monótona até dez. A cardinalidade do nível do meio não move a largura, porque o sorteio a tira do nível das folhas. O que resta é **plano de cobertura** e é decisão de desenho de medição, sem consumidor publicado conhecido: congelar a lista de modelos no repositório está fora de questão porque o roster é de fora e se move (§ 5.10) | antes de gerar a classe `ai`; as duas consequências que a linha alegava estão refutadas por medição |

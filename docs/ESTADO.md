...
| suíte | 172 arquivos / 3.115 testes (vitest) + 760 testes e 750 subtests (pytest, lab). Verde em rodada limpa e SOZINHA; com uma segunda corrida de vitest concorrente, dois a quatro arquivos de caminho selado batem no timeout de 20 s — dívida de § 7, não de política |
| dos quais, o avaliador | 2.498 — 1.738 em 46 arquivos de `benchmark/tests`, 760 no lab |
| typecheck | limpo |
...
| a **dependência de ordem está fechada nas três sondas diagnósticas**, e o conserto é o mesmo de `baseline_tfidf.py` com UMA autoridade: `common.canonical_fold_order` devolve a permutação `(rótulo, texto)` e as três sondas rendem canonicamente antes de dividir, espalhando os escores de volta às posições do chamador — `_stratified_out_of_fold_probabilities` (a que DECIDE, e a que `probe_lanes` também usa), `probe_length` e `probe_stylometry`. `shuffle=True` com semente fixa não consertava: ele permuta o que entrou. **A prova são baterias de permutação** sobre oito ordens, e a primeira versão delas foi **inútil e medida como tal**: montadas sobre `_length_correlated_corpus` (30 linhas idênticas por classe) os três mutantes sobreviveram verdes, porque a partição das dobras se move e o CONTEÚDO delas não. Refeitas sobre um corpo cuja linha difere dentro da classe, **oito** mutantes ficam vermelhos: as três renderizações, os dois espalhamentos de volta, a importância por permutação lida na ordem de entrada — que preserva a AUC e por isso escapava a uma bateria que só a olhava —, e a volta de `baseline_tfidf.py` a ter ordem canônica própria, que é o que faz de `common.canonical_fold_order` UMA autoridade e não duas | **feita** em 2026-08-22 |

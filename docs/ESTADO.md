...
| suíte | 172 arquivos / 3.102 testes (vitest) + 716 testes e 522 subtests (pytest, lab). Verde em rodada limpa e SOZINHA; com uma segunda corrida de vitest concorrente, dois a quatro arquivos de caminho selado batem no timeout de 20 s — dívida de § 7, não de política |
| dos quais, o avaliador | 2.441 — 1.725 em 46 arquivos de `benchmark/tests`, 716 no lab |
| typecheck | limpo |
...
| `evaluatorDigest` da árvore | `67886c9a827de8bbf832bc086f73c97dd86fab29bba240558b6a0e9b9b23fd8b` — 52 arquivos, recomputado pela função de produção e **lido por teste nomeado** (`digests.test.ts`, "is published in the ESTADO at the value the LIVE tree hashes to"), então este número não pode envelhecer em silêncio. Mover é barato enquanto `issuedAt` é nulo |

...
| | **a montagem abre os pools que lhe DÃO**: `load_humans(cand, reserved, pools, collision_sink)` percorre `pools`, cuja default é `HUMAN_POOL_FILES` — e `wikipedia_fresh`, a extração de 06/08, fica **fora** dela porque as 4.100 linhas sem carimbo rendem zero registro (§ 5.15). `--human-pool` é repetível na CLI, e a selecção é lida por `human_pool_selection`. **Três recusas e uma queda condicionada.** Pool **pedido** que não está no diretório recusa (`MissingHumanPool`) em vez de render zero linha em silêncio — a ausência do pool das reservadas continua legítima, e é estado com teste próprio. Selecção com pools que partilhem `candidateId` recusa (`OverlappingHumanPools`), com a origem contada sobre **toda linha lida** e não só sobre as que passam `REGISTER`, porque identidade de material não depende da moldura. Reservada **elegível** cujo id um pool já anexou **cai**, registada no `collision_sink` **no momento em que é decidida** — id, pool e `reservedRecord`, o **ordinal do registro** não vazio, que é o que distingue duas linhas sob um id; não é a linha física, porque `read_jsonl` descarta linha em branco antes da contagem. `main` passa o sink e imprime a **cardinalidade**; sem sink, a queda é impressa pelo próprio loader, num `finally`. A alegação é **«a queda é publicada no canal que o chamador escolheu»** e não mais larga: passar o sink é o chamador afirmar que o lê. Publicar só no fim perdia evidência — uma queda válida seguida de recusa por texto divergente deixava o sink intocado — e cai só se o **texto for idêntico**: texto divergente recusa (`ReservedTextDiffersFromThePool`), porque duas linhas distintas sob um id são conflito de material e escolher uma por igualdade de id apagaria conteúdo. A elegibilidade é avaliada **antes** da colisão, senão a contagem reporta perda que não houve. **Duas perguntas, dois conjuntos:** «que arquivo oferece este id?» é sobre toda linha lida, «este id já entrou?» é sobre as de facto anexadas — confundi-los tirava do corpus o documento cuja linha de pool `REGISTER` havia descartado. Medido no material real: 66 quedas, todas de texto idêntico, o dedup passou a remover **zero**, e a montagem constrói 4.100 | código |

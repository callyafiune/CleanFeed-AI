...
| suíte | 174 arquivos / 3.162 testes (vitest) + 987 testes e 972 subtests (pytest, lab). Verde em rodada limpa e SOZINHA; com uma segunda corrida de vitest concorrente, dois a quatro arquivos de caminho selado batem no timeout de 20 s — dívida de § 7, não de política |
...
| | **a montagem abre os pools que lhe DÃO**: `load_humans(cand, reserved, pools)` percorre `pools`, cuja default é `HUMAN_POOL_FILES` — e `wikipedia_fresh`, a extração de 06/08, fica **fora** dela porque as 4.100 linhas sem carimbo rendem zero registro (§ 5.15). `--human-pool` é repetível na CLI, e a selecção é lida por `human_pool_selection`. **Três recusas e uma queda, todas contadas:** pool pedido que não está no diretório **recusa** (`MissingHumanPool`) em vez de render zero linha em silêncio; selecção com pools que partilhem `candidateId` **recusa** (`OverlappingHumanPools`), com a origem contada sobre **toda linha lida** e não só sobre as que passam `REGISTER` — identidade de material não depende da moldura; e a linha reservada cujo id um pool já oferece **cai no load, contada em voz alta**. **As duas perguntas correm sobre conjuntos DIFERENTES, e confundi-los custou uma linha do corpus:** «que arquivo oferece este id?» é sobre toda linha lida, «este id já entrou?» é sobre as de facto anexadas — uma linha de pool que `REGISTER` descartou não entrou, e derrubar a reservada de mesmo id tirava o documento por inteiro enquanto a contagem dizia que um pool o oferecia. As três razões são a mesma: `dedup` colapsa por hash de texto e guarda a PRIMEIRA ocorrência, então sem elas é a **ordem** a decidir qual linha sobrevive, e as candidatas diferem no carimbo e na licença que decidem se ela é expressável. Medido no material real: 66 reservadas caem no load, o dedup passou a remover **zero**, e a montagem constrói 4.100 | código |
...
| **PAGA em 2026-08-25 — a montagem lia um nome constante, e o nome decidia a classe humana inteira.** `load_humans` percorria `("wikipedia_fresh",)`, então abria o pool de 06/08 e `human_record` recusava as 4.100 linhas dele: a classe humana da montagem era **zero** com o pool carimbado em disco ao lado. A lista passou a ser o terceiro parâmetro, com `--human-pool` repetível na CLI, e a mesma corrida sobre o material real mede as três pontas: default colhe 4.100 e constrói **4.100**; `pools=("wikipedia_fresh",)` colhe 4.100 e constrói **0**, todas `MissingExtractionRun`; os dois juntos são **recusados** nomeando o `candidateId` e os dois arquivos. **O que NÃO entrou, e a razão foi CORRIGIDA pelo cross-review:** eu escrevi que `diagnostic_probes.IN_FRAME_POOLS` e `ner_pilot.DEFAULT_POOLS` «já são default de argumento», e isso é falso para o primeiro — `IN_FRAME_POOLS` só é usado quando `--in-frame-pools` é passado, e sem a flag a sonda lê o **diretório inteiro**. `DEFAULT_POOLS` é de facto default de `--pools`. A decisão de escopo sobrevive por outra razão: as duas são **receitas históricas** e não autoridades da montagem — `IN_FRAME_POOLS` é a selecção que reproduz as taxas publicadas em § 5.7 sobre `benchmark/data/candidates`, e movê-la moveria a população de um número publicado. Baterias de **quatro** e de **cinco** mutações, nove mortas, restauração conferida por sha256 | fechada |
...
| **os fixtures de `main`, funil e triagem cobrem o PRIMEIRO pool da default**, `HUMAN_POOL_FILES[0]`, e com uma tupla de mais de um elemento medem um subconjunto — os arquivos seguintes viram `[]`. Hoje a tupla tem um elemento, então o subconjunto é o conjunto; o teste que percorre a tupla inteira existe e é só o do loader. Declarado em vez de consertado porque a alternativa — fixture por elemento em cinco chassis — é custo sem consequência medida enquanto a default for unitária | a segunda entrada em `HUMAN_POOL_FILES` |

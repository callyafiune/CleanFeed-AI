...
| suíte | 174 arquivos / 3.162 testes (vitest) + 996 testes e 972 subtests (pytest, lab). Verde em rodada limpa e SOZINHA; com uma segunda corrida de vitest concorrente, dois a quatro arquivos de caminho selado batem no timeout de 20 s — dívida de § 7, não de política |
...
| | as **três ilhas reservadas ficam com duas das oito tarefas**, `didatico` e `comentario`: a regra agrupa tarefas de cinco em cinco (00–04 `original`/`verbete`, 05–09 `parafrase`/`noticia`, 10–14 `resumo`/`feed`, 15–19 `didatico`/`comentario`) e as reservadas são as três últimas. **Não permutadas, e a razão é de desenho em três medições:** `task` não é membro de `SliceAxis`, logo nenhuma quantidade declarada publica recall por tarefa; o contraste `generatorExposure` já é **casado por tarefa**, porque as linhas de controle `seen` vivem DENTRO das ilhas reservadas (§ 5.12) e partilham as tarefas delas; e a reserva vigente espalhada por três pares daria ~83 por (família, tarefa), abaixo de qualquer poder por tarefa. O resíduo é **limitação do model card** (§ 7), não dívida de desenho. **Nada em código pina a uniformidade dos blocos de cinco**, e reabrir custa zero só até a primeira linha `ai`: depois dela as linhas escritas carregam digests de identidades que deixaram de existir, e corrigir é apagar material | AG |
| | as famílias **OpenAI vão ao NÚCLEO** — decisão do operador, revertendo a reserva por provedor: ChatGPT e Claude são as famílias que as pessoas de facto usam, e um detector cego para elas é cego para o caso dominante. A geração é pelo **codex CLI** (não API) e a Anthropic pelo **Claude Code** (não pelo agy, que fica com Gemini e `gpt-oss-120b`). O que a reversão FORÇA: `gpt-oss-120b-medium` é linhagem OpenAI e vai ao núcleo com ela — reservá-la seria reservar receita de fornecedor visto —, e com as duas fora a reserva fica **vazia**, que `declared_held_out_families` recusa ("'sem reserva' não é um estado que a governança consiga expressar"). Logo nomear uma quarta linhagem deixou de ser opcional e é **precondição da montagem**. A reserva escolhida é **local por ollama**, nas duas famílias da linha acima, e a alegação encolhe com ela: a fatia OOD passa a medir "linhagem de pesos abertos não vista" e não "provedor de fronteira ausente", com o viés otimista de um 7B quantizado declarado no model card. A reserva continua sendo política nomeada do slate (`OOD_RESERVED_FAMILIES`), não prefixo, e todo papel de família é **declarado** — `core`, `ood-reserved` ou `excluded` —, com censo dos pools conferido por guarda. **O que o código carrega hoje:** núcleo com as duas famílias OpenAI dentro, reserva com `qwen2_5-7b-q4km` e censo de 24 famílias sobre os DOIS diretórios de pool | OP |
| | `--provider` recusa na **argparse**, com as **cinco** lanes que `generate_ai.py` dirige como `choices` — subconjunto das seis congeladas, e o complemento é `claude-code` sozinho, conferido por nome: nenhuma chamada de provedor fora do slate é gasta. A lane `ollama` entrou como provedor DESTE script e não como arquivo próprio, para não haver segunda autoridade sobre ilha, semente, pareamento de comprimento, janela, PII, resume e lote; ela é a única com `seed`, a única cujo `harnessVersion` vem do SERVIDOR que responde e não de binário no PATH, e o host é conferido como loopback | AG |
| | **a montagem abre os pools que lhe DÃO**: `load_humans(cand, reserved, pools, collision_sink)` percorre `pools`, cuja default é `HUMAN_POOL_FILES` — e `wikipedia_fresh`, a extração de 06/08, fica **fora** dela porque as 4.100 linhas sem carimbo rendem zero registro (§ 5.15). `--human-pool` é repetível na CLI, e a selecção é lida por `human_pool_selection`. **Três recusas — uma delas com duas bocas — e uma queda condicionada.** Pool **pedido** que não está no diretório recusa (`MissingHumanPool`) em vez de render zero linha em silêncio — a ausência do pool das reservadas continua legítima, e é estado com teste próprio. `OverlappingHumanPools` tem **duas bocas**: selecção com pools que partilhem `candidateId`, com a origem contada sobre **toda linha lida** e não só sobre as que passam `REGISTER`, porque identidade de material não depende da moldura; e `candidateId` repetido **dentro de um só arquivo**, que recusa com mensagem própria — ali não há sobreposição de selecção nem ordem de pools que separe as duas linhas, e nomear o mesmo arquivo duas vezes descrevia um estado que não era o que acontecera. Reservada **elegível** cujo id **já foi admitido** — por um pool **ou pelo próprio arquivo das reservadas** — **cai**, registada no `collision_sink` **no momento em que é decidida**: id, `heldBy` e `reservedRecord`. `heldBy` é a coordenada que **detém** o id, e não se chama `pool` porque quem o detém pode ser o arquivo das reservadas; `reservedRecord` é o **ordinal do registro** não vazio, que é o que distingue duas linhas sob um id, e não é a linha física, porque `read_jsonl` descarta linha em branco antes da contagem. E cai só se o **texto for idêntico**: texto divergente recusa (`ReservedTextDiffersFromTheAdmittedRow`), porque duas linhas distintas sob um id são conflito de material e escolher uma por igualdade de id apagaria conteúdo. **A publicação da queda não depende do retorno normal do chamador**, e essa dependência era o defeito: `main` passa o sink e consome-o **depois** do retorno, então uma queda válida seguida de recusa por texto divergente ficava decidida e não chegava a canal nenhum — nem ao sink que ninguém leu, nem ao stdout, que se calava por haver sink. O piso do loader imprime quando **não há sink OU quando a corrida aborta**, e as duas condições são exclusivas do retorno normal com sink, logo nunca há publicação dupla. A alegação anterior — «a queda é publicada no canal que o chamador escolheu» — era mais larga que o código e está **retratada**. A elegibilidade é avaliada **antes** da colisão, senão a contagem reporta perda que não houve. **Duas perguntas, dois conjuntos, e uma autoridade só sobre a segunda:** «que arquivo oferece este id?» é `origem`, sobre toda linha lida; «este id já entrou?» é `admitidos`, e ele é actualizado em **toda** anexação — a de pool e a de reservada. Enquanto era povoado só no laço dos pools, o pareamento **reservada contra reservada** não tinha guarda nenhuma: medido, duas reservadas sob um id com textos diferentes entravam **as duas** em silêncio, e a jusante `enforce_unique_keys` renomeava a segunda — duas linhas humanas viravam **dois documentos**, contados sob a mensagem «colisão entre lanes», que é o que não eram. Incidência no material de hoje: **zero** (`reserved.jsonl` tem 4.054 registros e 4.054 ids distintos), e é a mesma razão pela qual a recusa por texto divergente foi construída para um caso que também não ocorre. Medido no material real: 66 quedas, todas de texto idêntico, com 66 ordinais distintos, o dedup remove **zero**, e a montagem constrói 4.100 | código |
...
| | a **janela do codex esteve ABERTA em 2026-08-24** e a metade de contrato do `llm-pii-screen` gastou duas rodadas nela: a adversarial cortada por filtro de conteúdo do provedor (`EXIT=1`, dois achados reais, sem veredito) e a defensiva com **APROVA** e `EXIT=0`. A lição de forma fica escrita porque custou uma rodada: prompt que pede *"nomeia o caminho e o comando"* ou *"escreve um chamador que escape"* é lido pelo filtro do provedor como pedido de exploração; a mesma pergunta cabe em *"esta invariante está imposta? por qual linha?"* e passa | OP |
...
**2026-08-25**, depois da re-extração do pool humano, da medição de expressabilidade que ela abriu e
da unidade que ensinou o pool de registro à montagem.

**A CLASSE HUMANA ESTÁ RESOLVIDA, e o que resta na frente é MATERIAL ou botão.** A montagem colhe as
4.100 linhas de `wikipedia_fresh_v2.jsonl` e constrói **as 4.100** (§ 5.15, § 7). Nenhuma porta de
código fica entre aqui e a geração: falta escrever o catálogo de controles, gastar a cota e apagar o
material antigo — os três dele.

**Ordem:**

1. **o CATÁLOGO DE CONTROLES da triagem — dele.** O código valida e recusa antes de gastar
...
2. **gastar a cota de geração, botão dele** — e agora com **três** condições que a corrida declara
...
3. **apagar o material antigo — dele.** As 400 linhas de qwen e as 2.135 mistas, com os snapshots já
...
**O que está FEITO e sai desta ordem:** a **re-extração do pool humano**, a **medição de
expressabilidade** dele — 4.100 de 4.100, geometria idêntica à do pool de 06/08 e a única chave que
difere é o carimbo (§ 5.15) — e a **lista de pools como argumento**, com selecção sobreposta recusada
(§ 7); a metade de contrato do `llm-pii-screen` (§ 5.14), a metade
...
botão de publicação externa — os três dele, os três de fase posterior. E a ratificação do perfil
`census-pii-screen-v1`, que é marco e espera a primeira execução.
...
| famílias geradoras nos pools | **24**, somando **6.583** linhas (`POOL_GENERATOR_FAMILIES`), medidas sobre os DOIS diretórios de pool — `data/candidates` e `data/candidates-f3` —, porque `--candidates-dir` aponta uma corrida para um deles e o censo tem de cobrir todo arquivo que uma corrida possa ler |
...
| linhas mistas escrevíveis hoje | **0** — as 2.135 recusam, 1.898 em `MissingRecipe` (a linha não carrega o digest do template de mistura) e 237 em `UnmappableLane` (a linha não nomeia lane e não carrega provedor) |
...
### 5.4c Os pais mistos por ILHA, e o rendimento que cada uma exige (2026-08-22) — **SUPERSEDITADA em 2026-08-24 pela § 5.15: esta população não é a da Fase 3, e os pais dela não podem ser registros**
...
**4.614** linhas — as 4.100 frescas mais **514** do pool reservado. As outras **66** do reservado saem
**no próprio loader**, contadas: o corpus morto de que o pool reservado vem foi construído da mesma
Wikipédia, então 66 ids coincidem com linhas frescas, e a cópia reservada cai porque o texto é idêntico
e ela não carrega licença de documento. A dedup exata por conteúdo tokenizado derruba, dessas,
**zero** — antes era ela que absorvia as 66, e com isso era a ordem de anexação que decidia qual
sobrevivia. Daí os **4.614** (4.100 + 514), **4.613** depois da poda intra-pool (a linha derrubada é fresca) e **4.607** depois da poda global
...
**A sonda de lane cobre TRÊS lanes e não mais.** As lanes medidas são `codex` (1.402 linhas),
`gemini-api` (751) e `agy` (419); de `fable` só existe `lane_parents_fable.jsonl`, com 60 pais humanos.
Isso é fato de Fase 3 e não erro de contagem: qualquer leitura que dê a D13/W2 uma quarta lane é falsa.
...
São as **quatro lanes que produzem material**, das seis do vocabulário congelado: `gemini-api`
fica declarada e sem material novo, e `gemini-cli` não tem plano de geração. Uma linha por modelo,
e a coluna **verificação** é o que separa medido de inferido.

**O que é congelado e o que não é.** A pré-inscrição congela a FORMA da lane — o canal, o nome da
escala de effort, os níveis dessa escala e as fontes de effort que a lane pode produzir — e essas
quatro coisas são lidas por `schema.ts` e pelo montador. Os **rosters** não: os ids de modelo são
de fora e se movem, e nenhum deles existe no repositório. Logo o que a corrida gravar vale mais
que as tabelas abaixo, e as tabelas existem para que a corrida não seja escrita de memória.

**As linhas de lane vigentes**, que são o que o código lê:

| lane | canal | knobs de amostragem | escala de effort | níveis | fontes |
|---|---|---|---|---|---|
| `agy` | `agent-cli` | não | `agy-model-id-tier` | low, medium, high | `model-id`, `flag`, `not-supported` |
| `claude-code` | `agent-cli` | não | `claude-code-effort` | low, medium, high, xhigh, max | `flag`, `not-supported` |
| `codex` | `agent-cli` | não | `codex-reasoning-effort` | low, medium, high, xhigh, max, ultra | `flag`, `provider-default` |
| `gemini-api` | `api` | sim | — | — | `not-supported` |
| `gemini-cli` | `cli` | não | — | — | `not-supported` |
| `ollama` | `local-runtime` | sim | — | — | `not-supported` |

Duas propriedades da tabela são **derivadas do canal e conferidas contra ele**, e é a separação que
o canal existe para expressar: o eixo `harnessVersion` aplica-se em todo canal que não seja `api`,
e os knobs de amostragem pertencem a `api` e a `local-runtime`. `ollama` é a única lane em que as
duas valem, e ler uma da outra é o que a tornava inescrevível. **Configurabilidade de effort não é
propriedade da lane**: `configurable` é do REGISTRO — se aquele effort era ajustável — e a lane
apenas o LIMITA, porque ajustável exige que ela ofereça a fonte `flag`. Qual dos modelos de uma
lane aceitou a flag é por modelo, e aí o registro é a autoridade.
...
A faixa `low…xhigh` é a **interseção dos seis** e é o plano de cobertura, logo 6 × 4 = **24
combinações**. Não é o que a política declara e não deve ser: `codex.effortLevels` carrega os
**seis** níveis da escala, porque o campo são os níveis DA ESCALA e uma lista curta ali é descrição
falsa dela, não teto. O teto é do plano.
...
É a terceira ocorrência de propriedade por modelo que a linha de lane não expressa — depois do
effort do agy e dos níveis do codex, e as duas primeiras a forma da lane já resolve. Esta não: "o
runtime injeta system prompt" é verdade do qwen e falsa do llama3, então não é fato de lane e não
é o que faz do `local-runtime` um canal próprio. O que faz é a versão do runtime mais os knobs.
...
Os **ids de conteúdo dos dois modelos são conferíveis agora** e batem com esta tabela
(`845dbda0ea48` e `9b8f3f3385bf`, medidos), o que faz desta a única lane em que a identidade dos
pesos não depende de acreditar em ninguém. A **versão do runtime não**: o binário instalado está em
**0.32.15** e as 400 linhas gravaram **0.32.6**, então a deriva de que esta seção avisa aconteceu, e
a reprodutibilidade daquelas linhas contra esta máquina deixou de valer. **O que falta da segunda
família são LINHAS**: o tag está decidido, os pesos estão em disco, o id é conhecido, e nenhuma
linha foi gerada. Ela não é opcional — reserva com uma família só não estima dispersão, e reserva
...
**O QUE JÁ ESTÁ EM CÓDIGO (2026-08-24), e o custo de contrato MEDIDO.** O perfil está
**pré-inscrito**: `benchmark/assurance-profile.ts` é o registro fechado, com **dois** membros —
`full-human-review-v1`, que é a semântica que o selo já impunha, e `census-pii-screen-v1`, com
`activation.state: "pre-registered"`. `assuranceProfile` é campo do manifesto, **obrigatório** em
`release` e **recusado** em `infrastructure-only` (`DATASET_ASSURANCE_INVALID` nas duas direções), e o
`dataset-audit.json` o publica — `null` quando não há alegação de `release` a qualificar. `sealDataset`
recusa por nome o selo sob perfil pré-inscrito (`DATASET_ASSURANCE_INACTIVE`, listando o que a
ativação exige) e, sob perfil ativo cuja alegação é censo, recusa o corpus em que **qualquer** registro
não carregue a corrida `llm-pii-screen` (`DATASET_ASSURANCE_UNSUPPORTED`, com contagem e ids). A
recusa da alegação de revisão (`DATASET_REVIEW_INVALID`) passou a ser perguntada **só** sob perfil que
afirma leitura humana por registro — sob o censo ela refutaria o corpus por ser exactamente o que
declara. **O custo medido:** `evaluatorDigest` moveu **uma** vez, de
`8b3efb2b…` para `d108609845e5a42d7b144d990f1237ea7f3eb1847f715cc29833869a6b3ef554`, e
`EVALUATOR_FILES` foi de **52** para **53** arquivos; `DatasetAudit` ganhou um campo, o que obrigou
**12** sítios de fixture a declará-lo; `AUTOMATED_FILTERS` ganhou `llm-pii-screen` e
`reviewClaimSupport` **não** mudou. **A ativação é literal versionado** e o giro dele move o digesto
...
não ao literal congelado, porque o registro é parâmetro ali. E a guarda da costura passou a
**descobrir** os chamadores varrendo `benchmark/`, `src/`, `scripts/` e `contracts/`, em vez de
enumerar cinco módulos por nome — dos cinco enumerados, **quatro não tinham chamada nenhuma**, e um
módulo de produção novo não estaria na lista. O caso medido: um módulo novo com chamada de cinco
argumentos deixava a guarda antiga verde e derruba a nova. O registro é injetável
em `sealDataset` **só** como costura de teste, e uma guarda lê os sítios de chamada e recusa um quinto
argumento em qualquer módulo de produção.
...
fica como evidência e a montagem **não** o abre: `HUMAN_POOL_FILES` não o nomeia, e apontar `pools`
para ele colhe as mesmas 4.100 linhas e constrói **zero** — medido (§ 3.3, § 7).

#### A geometria, medida no pool de registro

| medição | valor |
|---|---:|
| na janela do extrator (`common.MINIMUM_WORDS`–`MAXIMUM_WORDS`) | **4.100** (todas) |
| ilhas cobertas | **20** de 20 |
| menor ilha | **187** (`ilha_01`, `ilha_04`) |
| maior ilha | 225 |
| rendimento de banda mínimo para a cota de 100 por ilha | **53,5 %** |

**Contra a § 5.4c, que media `reserved.jsonl`:** 2.578 pais, menor ilha 108, rendimento mínimo
**92,6 %**. A § 5.4c fica **superseditada**: a população dela não é a da Fase 3, e os pais dela não
podem ser registros (ver abaixo). E a frase da § 5.13 sobre «2.578 pais admissíveis contra 2.000» lê-se
agora como **4.100 contra 2.000**.

#### A geração que está em disco é de outra moldura

`benchmark/data/candidates/` — o diretório antigo — carrega 4.048 linhas `ai` e 2.135 mistas. Medido
lá:

| medição | valor |
|---|---:|
| linhas `ai` que nomeiam semente ausente do pool humano daquele diretório | **2.319** |
| … semente presente | 253 |
| … sem semente declarada (a reserva; `load_ai` descarta o `pairedWith` dela) | 1.476 |
| mistas cujos pais vivem **só** em `reserved.jsonl` | **2.135** (todas) |
| … dessas, com `parentFamily` fora da moldura | 1.597 (`ptso_qa` 1.111, `carolina_*` 486) |

As sementes ausentes são `src_ptso_*` e companhia — fontes que a moldura já não declara. Por família:
`gpt-5.6-luna` 1.294, `gemini-3.5-flash-lite` 414, `gemini-3.5-flash-low` 320, `gemini-3.1-flash-lite`
190, `gemini-3.5-flash-medium` 99, `gemini-3-flash-preview` 2. **A leitura:** este material não é
candidato à Fase 3 por duas razões independentes — a moldura e o pai —, e as 2.135 mistas somam-se às
que a § 7 já dá por regeráveis.

#### O que o código faz agora com isso, e o que ele não cria

A linha derivada órfã sai **antes da selecção** (`drop_orphan_derived_rows`, contada por classe), a
selecção humana **protege as âncoras**, a cascata da triagem alcança a classe `ai`, e um guarda
pós-construção (`drop_records_whose_parent_is_absent`) prova que a combinação fechou. A pista mista
passou a **ler a forma de candidato** e a **recusar** pai que não nomeia aquisição
(`parent_refusal_reason`), com a itemização impressa antes da primeira chamada. Medido nos dois
arquivos: `reserved.jsonl` dá `{pai-nao-nomeia-aquisicao: 2.578, nao-e-classe-humana: 1.476}` e
**zero** admissíveis; o pool da Fase 3 dará 4.100 admissíveis **depois** de carimbado.

Nada disto **cria** material. A ordem que ele impõe é: **re-extrair o pool humano → medir a
expressabilidade → gerar contra esse pool**. Gerar antes da primeira medição é pagar por linhas cujo
pai o corpus não vai conter.
...
pai), e emendar `assertDerivedParentsResolve` desfaz a condição que mantém semente e geração na mesma
partição.

### 5.16 Os controles, os gates e o recibo da triagem, em código (2026-08-24)

A metade do lab do `llm-pii-screen` está fechada em código. O que a § 7 listava como aberto —
`S_control` por estrato, gates adversariais por pares, indistinguibilidade, o recibo e o adaptador
real do triador — existe, com **61 casos** que não falam com provedor nenhum e **17 mutações, 17
mortas**.

**O que cada gate mede, e o que ele NÃO decide:**

| gate | estatística | piso/teto | aborta? |
|---|---|---|---|
| `S_control` por estrato | limite **inferior** de Wilson unilateral 95 % | o piso do subtipo (0,95 / 0,80 / 0,75 / 0,70) | sim, nomeando o estrato |
| sham | limite **superior** de Wilson | `SHAM_FALSE_POSITIVE_CEILING` = 0,10, **um** teto sobre a união | sim |
| par adversarial | limite inferior, por vetor | `CORRECT_PAIR_RATE_FLOOR` = 0,80 | sim, nomeando o vetor |
| indistinguibilidade | acurácia do discriminador | azar ± 0,10, **nas duas direções** | sim |

**O gate é sobre SINALIZAR, e está escrito no recibo** (`gateIsOnFlagging`): o que protege o corpus é
o sinalizar — D-12 dropa todo sinalizado, com subtipo certo ou errado —, então um piso que exigisse a
categoria correcta mediria competência taxonômica, que é outra alegação. `subtypeAgreement` é
publicado por estrato como **diagnóstico** e não gateia nada. E o par de gates é coerente por
construção: um triador que sinalize tudo alcança sensibilidade 1,0 em todo estrato e cai no teto de
sham — medido por caso.

**O dinheiro tem duas travas antes da primeira chamada:** o catálogo é conferido contra os literais do
protocolo (contagem por subtipo, pareamento 1:1, fração escrita à mão, os quatro vetores, os cem
pares), e `--confirm-calls` tem de casar **exactamente** com o total derivado por
`planned_call_count`. As duas recusam antes de a primeira chamada sair.

**O prompt é derivado da taxonomia pré-inscrita**, não uma segunda cópia dela, e o digesto que o
recibo publica **move-se com ela** — medido por caso que acrescenta um subtipo e vê o digesto andar. O
digesto enquadra as partes com o comprimento à frente, e não a concatenação delas: duas divisões da
mesma cadeia dariam o mesmo valor se fossem coladas, e isso tornaria duas pré-inscrições distintas
indistinguíveis no recibo.

**O parser da resposta é fechado, e a divisão de trabalho é declarada:** ele confere a **forma** (chave
a mais, tipo errado, prosa livre recusam; cerca de código é aceita), e o **vocabulário** é do censo
(`TriagerNamedUnknownSubtype`). Um subtipo inventado passa o parser de propósito e morre no censo —
duas validações do vocabulário poderiam discordar.

**O recibo não se constrói sobre gate reprovado**, e a recusa nomeia **todos** os que caíram, não o
primeiro. Ele carrega a alegação verbatim, a lista do que ela **não** afirma, `prevalenceBound: null`
e os **nove riscos nomeados**, que entraram na pré-inscrição (`NAMED_RISKS`) para serem citados de lá.
Mover o protocolo move `protocol_digest()`: legítimo agora porque **nenhuma execução correu**, e
emenda com recibo a invalidar depois da primeira.

**O que fica aberto, e é material:** o **catálogo de controles não existe**. São 660 injeções + 660
shams, 320 sondas adversariais e 100 pares, com **ao menos 1/3 das injeções de cada subtipo escritas à
mão** e o resto de um gerador de família **diferente** do triador. E uma execução **reprovada** não
deixa JSON: os vereditos dos quatro gates são impressos e os logs ficam em disco, mas o recibo é o
artefato da alegação, e não há alegação a fazer.
...
- **"uma família num só cluster de template colapsa o intervalo, e a largura é 0,0000 contra 0,6000
  com dois"** (registro de 2026-08-22, retratada no mesmo dia): o 0,0000 era artefato do fixture —
  no caso de um template eu dei taxa CONSTANTE a todas as linhas da família, então a variabilidade
  era zero por construção. A remedição com a variabilidade intra-cluster fixa não mostrou tendência
  monótona (0,2167 e 0,2167 com um e dois templates), e ela **também** não isola a cardinalidade da
  partição posicional dos acertos — então nem o efeito nem a ausência dele estão estabelecidos. A
  guarda escrita sobre a alegação (`assert_every_family_spans_two_template_clusters`, com classe,
  constante e dois testes) foi **retirada inteira** no commit seguinte. E a frase "de dois em diante
  é o √n comum", da mesma entrada, era lei assintótica inventada de três pontos que seguem 1,2/n no
  fixture. O que sobrou de válido está na § 7 e é mais estreito
...
| dívida | vence |
|---|---|
| a **admissibilidade do intervalo tem teto de massa condicionante**, derivado e não pré-inscrito. `bootstrap.ts` recusa a estatística cuja contagem descartada passa `floor((1 − descriptiveConfidence)/2 × pedidas)` e **omite só o simultâneo** quando passa `floor(simultaneousAlpha × pedidas)` — por estatística, antes de qualquer percentil. **O que isto compra, e o que não compra:** o lema é `|P(X≤x) − P(X≤x | finito)| ≤ p`, então o nível do quantil lido em `q` cai em `[q−p, q+p]`; com `p` logo abaixo de `alpha` a cauda do pior caso chega a `alpha + p − alpha·p` ≈ **2× o nominal** (0,0499 contra 0,025). É **cota de degradação** sob fator dois e critério de admissibilidade do avaliador — **não** prova de cobertura. Garantia incondicional exigiria ranks ajustados, e essa decisão fica de fora | **feita** em 2026-08-23 |
| o **gate confunde tentativas PEDIDAS com réplicas FINITAS, e o piso por fase está errado.** `simultaneousEffortFailure` compara réplicas finitas com `MINIMUM_DECLARED_REPLICATES = bootstrapReplicates.pilot` **em qualquer fase**. Consequência medida: à escala piloto (10.000 pedidas) uma única réplica não finita reprova o gate por esforço insuficiente — tolerância de descarte **0 %**; à escala release (100.000 pedidas contra o mesmo piso de 10.000) a tolerância é **90 %**. Ninguém escolheu nenhum dos dois: é função-degrau por fase, não política. O teto do `bootstrap.ts` **acrescenta** a condição correta e **não substitui** esta — separar as duas quantidades no gate, aplicar o piso pré-inscrito da fase, propagar causa tipada para descarte excessivo (hoje ele chega como `missing-simultaneous-interval`, que esconde a causa), publicar a fração como auditoria, e decidir à parte se os ranks devem ser ajustados | antes da primeira corrida certificadora — é ela que corre à escala release |
...
| **PAGA em 2026-08-25 — a montagem lia um nome constante, e o nome decidia a classe humana inteira.** `load_humans` percorria `("wikipedia_fresh",)`, então abria o pool de 06/08 e `human_record` recusava as 4.100 linhas dele: a classe humana da montagem era **zero** com o pool carimbado em disco ao lado. A lista passou a ser o terceiro parâmetro, com `--human-pool` repetível na CLI, e a mesma corrida sobre o material real mede as três pontas: a selecção default **sobre `candidates-f3`** colhe **4.614** — as 4.100 do pool mais as reservadas em moldura que sobraram — e dela `human_record` constrói **4.100**, recusando 514 por `MissingDocumentLicense`, que é o estado registrado do pool das reservadas (a corrida sem `--candidates-dir` recusa, ver a dívida do diretório); `pools=("wikipedia_fresh",)` colhe 4.100 e constrói **0**, todas `MissingExtractionRun`; os dois juntos são **recusados** nomeando o `candidateId` e os dois arquivos. **O que NÃO entrou, e a razão foi CORRIGIDA pelo cross-review:** eu escrevi que `diagnostic_probes.IN_FRAME_POOLS` e `ner_pilot.DEFAULT_POOLS` «já são default de argumento», e isso é falso para o primeiro — `IN_FRAME_POOLS` só é usado quando `--in-frame-pools` é passado, e sem a flag a sonda lê o **diretório inteiro**. `DEFAULT_POOLS` é de facto default de `--pools`. A decisão de escopo sobrevive por outra razão: as duas são **receitas históricas** e não autoridades da montagem — `IN_FRAME_POOLS` é a selecção que reproduz as taxas publicadas em § 5.7 sobre `benchmark/data/candidates`, e movê-la moveria a população de um número publicado. Baterias de **quatro** e de **cinco** mutações, nove mortas, restauração conferida por sha256 | fechada |
| a **default da montagem e o diretório default não se encontram**, e a consequência é uma recusa em vez de um erro silencioso: `CAND` é `benchmark/data/candidates` — o diretório **pré-emenda** — e o pool de registro vive em `candidates-f3`, então uma corrida sem `--candidates-dir` bate em `MissingHumanPool`. **Alinhar mexendo em `CAND` foi RECUSADO, medido:** `load_ai` e `load_mixed` leem os pools do diretório antigo e silenciam arquivo ausente (`read_jsonl` devolve `[]`), então mover `CAND` trocaria uma recusa alta na classe humana por vazio silencioso nas duas classes geradas. A recusa é a resposta vigente; o alinhamento espera o diretório único, que é o item 3 da § 3.9 (apagar o material antigo). A silenciosidade de `load_ai`/`load_mixed` fica nomeada e não é desta unidade | quando o material antigo sair, ou a unidade que der a `load_ai`/`load_mixed` a mesma recusa |
| **os fixtures de `main`, funil e triagem cobrem o PRIMEIRO pool da default**, `HUMAN_POOL_FILES[0]`, e com uma tupla de mais de um elemento o fixture escreve só o primeiro — e desde `MissingHumanPool` isso deixou de ser um `[]` silencioso e passou a ser **recusa** no segundo nome, então os cinco chassis ficariam vermelhos em vez de medir menos. Hoje a tupla tem um elemento; o teste que percorre a tupla inteira existe e é só o do loader. Declarado em vez de consertado porque a alternativa — fixture por elemento em cinco chassis — é custo sem consequência medida enquanto a default for unitária | a segunda entrada em `HUMAN_POOL_FILES` |
...
| `format:check` está **VERDE**: os dois `it(...)` de `benchmark/tests/consume-holdout.test.ts` com terceiro argumento (`TIMEOUT_MS`) foram formatados — **78 inserções / 68 remoções**, exactamente a medição que esta linha publicava, formatação e nada mais. O arquivo não está em `EVALUATOR_FILES` e o `evaluatorDigest` **não se moveu**, conferido pela função de produção antes e depois. `npm run verify` deixa de parar aqui | **feita** em 2026-08-21 |
| a **forma da linha de lane** admite as seis lanes, e o que fica em aberto é material e não forma. O canal separa duas coisas antes colapsadas — `harnessVersion` aplica-se em todo canal que não seja `api`, os knobs de amostragem pertencem a `api` e a `local-runtime`, e `ollama` é a única lane em que as duas valem. `decodingConfigurable` é conferido contra o canal por IGUALDADE, nas duas direções: negar os knobs de um canal que os expõe descarta o `seed`, que é o único campo que torna uma geração reproduzível. A configurabilidade de effort saiu da linha de lane: `configurable` é do REGISTRO — se aquele effort era ajustável — e a lane apenas o LIMITA, porque ajustável exige que a lane ofereça a fonte `flag`. Não é derivada da fonte, e a diferença é medida: derivar recusaria `provider-default` com `configurable: true`, forma que a política congelada admitia no codex. O espaço de registros está **enumerado por teste**, doze formas admitidas, e as seis anteriores à emenda estão todas dentro — invalidação retroativa **nenhuma**, medida e não argumentada. `effortLevels` passou a ser a escala completa (`codex` com seis níveis), porque o campo são os níveis da escala e uma lista curta é falsidade; e `not-supported` é fato do registro — a lane, o modelo ou a forma de invocação não tinham o controle —, o que torna escrevíveis as 122 linhas da sessão interativa. Uma lane sem escala oferece `not-supported` e **nada mais**, por igualdade: qualquer outra fonte carrega nível, e nível precisa de escala. Bateria de **nove** mutações, cada uma vermelha no teste nomeado, mais uma que sobreviveu verde e foi consertada | **feita** em 2026-08-21 |
| a **declaração de que o número OOD é medido sobre duas das oito tarefas** não existe onde a alegação é lida: as três ilhas reservadas carregam `didatico` e `comentario`, e a disposição está **decidida** e não é dívida (§ 3.3). O que falta é escrever a limitação | Fase 6, no model card |
| a pista mista **realiza as três operações**, e o que sobra dela é menor e está medido. `MIX_TEMPLATES` serve **63** receitas: as **60** identidades `mix-<operação>-ilha-NN`, compostas de geometria × intenção × registro com o nível como parâmetro preenchido, mais as **três** `mix_*_v1` legadas, que ficam porque os pools em disco foram escritos sob elas. Os 63 têm 63 digests distintos, e a igualdade é conferida também na fronteira do `argparse` — sessenta nomes sobre corpos repetidos passariam a conferência por NOME e deixariam a partição de ilha NOMINAL. O laço de `--generate` itera as **20 células** que `mix_cells()` deriva, toma pai só do próprio bloco de semente, e a linha grava `mixOperation` e `mixLevel` no TOPO do registro de pool — nunca dentro de `mixture`, que é vocabulário FECHADO do esquema selado. **Três resíduos, declarados:** (i) as sondas de eco de `artifact_gate` derivam só de `RECIPES`, então a prosa de **geometria** e de **intenção** dos 60 não tem sonda — a de registro tem, porque é byte a byte a do slate de geração —, e crescê-las mudaria a taxa medida contra o teto **pré-inscrito** de 2 %, logo é medição com unidade própria; (ii) **`make_mixed_agy.py` passou a pedir por CÉLULA** — `--island` obrigatória, pai só do bloco de semente da ilha, identidade do slot da ilha, nudge no nível vizinho, e `mixOperation`/`mixLevel` no par —, então as linhas dela pertencem a ilha e a montagem as aceita; `make_mixed_codex.py` **continua** a fazer edit genérico pinando `mix_edit_v1`, e é a razão de `--assume-template` só admitir as receitas **sem** operação; (iii) **medido em 2026-08-22, e o número por ilha muda a leitura** (§ 5.4c): 2.247 pais admissíveis contra 2.000 de cota é excedente global de 12,4 %, mas **quatro ilhas ficam abaixo da cota antes de qualquer perda de banda** — `ilha_08` com 92, `ilha_12` com 94, `ilha_13` com 95 e `ilha_14` com 99, contra 100 —, e as quatro exigiriam rendimento de banda acima de 1. Déficit somado de 20 linhas, e a folga de 267 nas outras dezesseis **não transfere**, porque cada ilha toma só o próprio bloco de semente. `--generate` passou a imprimir o déficit antes da primeira chamada; é aviso e não recusa, porque o remédio é escolha de quem coleta | (i) e (iii) antes de fechar a Fase 3; (ii) inalterado |
...
| a **dependência de ordem está fechada nas três sondas diagnósticas**, e o conserto é o mesmo de `baseline_tfidf.py` com UMA autoridade: `common.canonical_fold_order` devolve a permutação `(rótulo, texto)` e as três sondas rendem canonicamente antes de dividir, espalhando os escores de volta às posições do chamador — `_stratified_out_of_fold_probabilities` (a que DECIDE, e a que `probe_lanes` também usa), `probe_length` e `probe_stylometry`. `shuffle=True` com semente fixa não consertava: ele permuta o que entrou. **A prova são baterias de permutação** sobre oito ordens, e a primeira versão delas foi **inútil e medida como tal**: montadas sobre `_length_correlated_corpus` (30 linhas idênticas por classe) os três mutantes sobreviveram verdes, porque a partição das dobras se move e o CONTEÚDO delas não. Refeitas sobre um corpo cuja linha difere dentro da classe, **onze** mutantes ficam vermelhos: as três renderizações, os dois espalhamentos de volta, a importância por permutação lida na ordem de entrada — que preserva a AUC e por isso escapava a uma bateria que só a olhava —, e a volta de `baseline_tfidf.py` a ter ordem canônica própria. **A autoridade única é afirmada por substituição e não por mutação de valor**: trocar `common.canonical_fold_order` por uma que inverte e exigir que os **quatro** leitores a sigam — o renderizador e o guardião do baseline, a sonda que decide e a de estilometria —, porque um `sorted` local semanticamente igual passaria por qualquer bateria de invariância. E o guardião de `baseline_tfidf.py` também delega — um guardião que redefine "canônico" é uma terceira definição —, com mutante próprio, porque a substituição da autoridade só o alcança pelo caminho que renderiza E confere | **feita** em 2026-08-22 |
...
| a família reservada da v1 é `qwen2_5-7b-q4km`, e o critério de facilidade foi exercitado contra `gpt-5_6-luna`, que é modelo de fronteira **e é núcleo**. A comparação que o critério existe para decidir — reservada contra núcleo — só é possível medindo sobre as 400 linhas que a reservada tem hoje, e não foi feita | Fase 3, item 2 |

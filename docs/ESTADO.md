...
| suíte | 174 arquivos / 3.162 testes (vitest) + 977 testes e 970 subtests (pytest, lab). Verde em rodada limpa e SOZINHA; com uma segunda corrida de vitest concorrente, dois a quatro arquivos de caminho selado batem no timeout de 20 s — dívida de § 7, não de política |
...
| as **três ilhas reservadas carregam duas das oito tarefas** de geração — `didatico` e `comentario` —, porque a regra agrupa tarefas de cinco em cinco (ilhas 00–04 `original`/`verbete`, 05–09 `parafrase`/`noticia`, 10–14 `resumo`/`feed`, 15–19 `didatico`/`comentario`) e as reservadas são as três últimas. **Não permutadas, e a razão é de DESENHO e não de preço — retificado em 2026-08-23.** Três medições: (i) `task` **não é eixo de fatia** — `SliceAxis` tem dez membros e nenhum é a tarefa —, então nada na alegação publica recall por tarefa e não há confundimento a consertar numa quantidade declarada; (ii) o contraste seen/unseen já é **casado por tarefa**, porque as 100 linhas de controle `seen` vivem DENTRO das ilhas reservadas (§ 5.12) e partilham as tarefas delas; (iii) 500 linhas de reserva (2 × 250) espalhadas por três pares dariam ~83 por (família, tarefa), **abaixo de qualquer poder por tarefa** — permutar compraria um marginal sobre mistura mais larga e nada mais. **O custo de permutar hoje é ZERO**, e a razão antiga ("trocaria os 40 digests") não se sustentava: os únicos `promptTemplateDigest` literais na árvore são do corpus MORTO (`ai_openai.batch.json` e o `source-manifest` privado), nada rastreado pina os 40 do slate novo, e nenhuma linha `ai` existe. O que fica é **limitação a declarar no model card** — o número OOD é medido sobre duas tarefas de oito — e não dívida a pagar | prazo, não preço: permutar deixa de custar zero na primeira linha gerada |

...
| suíte | 172 arquivos / 3.056 testes (vitest) + 668 testes e 437 subtests (pytest, lab). Verde em rodada limpa e SOZINHA; com uma segunda corrida de vitest concorrente, dois a quatro arquivos de caminho selado batem no timeout de 20 s — dívida de § 7, não de política |
...
| `evaluatorDigest` da árvore | `1bd5f072213defea6abb5678ee00531514f8542ec2fed30d47738c51e1c57b71` — 52 arquivos, recomputado pela função de produção e **lido por teste nomeado** (`digests.test.ts`, "is published in the ESTADO at the value the LIVE tree hashes to"), então este número não pode envelhecer em silêncio. Mover é barato enquanto `issuedAt` é nulo |
...
| dívida | vence |
|---|---|
| **o slate não cumpre o plano de ilhas, e por isso nenhuma linha pode ser gerada**: `island_plan` recusa TODA ilha — `RECIPES` declara **4** identidades de template contra as **100** que o plano passa a exigir. **Deixou de ser pendência do operador**: o número foi ratificado em 2026-08-12 (§ 3.3), e o que falta é trabalho de agente — crescer `RECIPES` até servir as 100 e emendar `_island()` para três slots de mistura. Encolher a cota **não** reduz o número de ilhas: o piso é sobre fração (`1/n ≤ 0,05 + 0,02`), logo é livre de escala. É a guarda funcionando antes da cota, não defeito | unidade seguinte; a **cota de geração** que ela vai gastar continua sendo chave do operador |

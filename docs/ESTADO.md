...
| dívida | vence |
|---|---|
| **o slate não cumpre o plano de ilhas, e por isso nenhuma linha pode ser gerada**: `island_plan` recusa TODA ilha — o plano declara 40 identidades de template e `RECIPES` declara **4**. Ou `RECIPES` cresce até cobrir o plano, ou o plano é emendado para menos templates por ilha (mínimo 15 ilhas, um template cada). É a guarda funcionando antes da cota, não defeito | **decisão do operador**, antes da Fase 3 item 2 |
| a **guarda pré-cota da classe MISTA não existe**: os três `make_mixed*.py` usam um template único de mistura, e medido isso leva o corpo de 20 componentes a **um de 100 %** — pais mistos espalhados, também. O plano declara `mixingTemplate` por ilha e a guarda o confere, mas nada o impõe aos três produtores. A cota mista ainda não foi gasta, então a janela está aberta | unidade seguinte, antes de gerar a classe mista |

...
| suíte | 172 arquivos / 3.115 testes (vitest) + 757 testes e 750 subtests (pytest, lab). Verde em rodada limpa e SOZINHA; com uma segunda corrida de vitest concorrente, dois a quatro arquivos de caminho selado batem no timeout de 20 s — dívida de § 7, não de política |
| dos quais, o avaliador | 2.495 — 1.738 em 46 arquivos de `benchmark/tests`, 757 no lab |
| typecheck | limpo |
...
| **nada cruza modelo e effort com template, e a lacuna é de PLANO.** Medido: uma ilha carrega `templates`, `mixingTemplates`, `seedBlock`, `lines` e `reserved`; uma receita carrega `task`, `register`, `template` e `weight`. **Nenhum dos dois nomeia modelo ou effort** — os dois são argumentos POR CORRIDA. A identidade de PROMPT é planejada e particionada por ilha (40 templates, `promptTemplate` na união do split); a de GERAÇÃO — 24 no codex, 12 no agy, 20 no Claude Code — é o que a linha de comando fizer, e nada garante que um modelo apareça em mais de uma ilha. **A CONSEQUÊNCIA FOI MEDIDA em 2026-08-22 e não é a que esta linha dizia.** (i) A ameaça de ATRIBUIÇÃO por fatia **não existe**: nenhuma fatia é chaveada por família, modelo ou effort — `generatorExposure` tem dois valores, `seen` e `unseen`, e mais nada (`benchmark/slices.ts`). (ii) O dano real é no INTERVALO: a unidade de `ai-recall` aninha `promptTemplate` DENTRO de `generatorFamily`, então uma família confinada a um cluster de template não contribui variabilidade no nível do meio. Medido pela função de produção (10.000 réplicas, mesma semente, mesmas linhas, só a estrutura a variar): largura do intervalo de 95 % em **0,0000** com um template por família — o intervalo COLAPSA num ponto — e **0,6000** com dois; de dois em diante o estreitamento é o √n comum. Logo o piso é **dois** e é qualitativo. (iii) E nada publicado o denunciava: `units` é idêntico nas duas estruturas e `degenerate` é verdadeiro nas duas de qualquer forma, porque o nível mais interno tem uma unidade por linha em todo corpo realista. **Fechado por `assert_every_family_spans_two_template_clusters`**, chamada em `main()` sobre o corpo inteiro depois da poda das famílias magras, com o piso em `MINIMUM_TEMPLATE_CLUSTERS_PER_FAMILY`. O que fica aberto é o **plano de cobertura** de modelo × effort, que é outra pergunta e é de desenho de medição: nada exige que um modelo apareça em mais de uma ilha, e congelar a lista de modelos no repositório está fora de questão porque o roster é de fora e se move (§ 5.10) | a guarda do intervalo **feita** em 2026-08-22; o plano de cobertura, antes de gerar a classe `ai` |

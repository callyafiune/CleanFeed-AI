...
| suíte | 172 arquivos / 3.097 testes (vitest) + 712 testes e 518 subtests (pytest, lab). Verde em rodada limpa e SOZINHA; com uma segunda corrida de vitest concorrente, dois a quatro arquivos de caminho selado batem no timeout de 20 s — dívida de § 7, não de política |
| dos quais, o avaliador | 2.432 — 1.720 em 46 arquivos de `benchmark/tests`, 712 no lab |
| typecheck | limpo |
...
| dívida | vence |
|---|---|
| **o slate não cumpre o plano de ilhas, e por isso nenhuma linha pode ser gerada**: `island_plan` recusa TODA ilha — `RECIPES` declara **4** identidades de template contra as **40** que `island_plan` confere — dois templates de geração por ilha vezes vinte ilhas. As **100** identidades são do CORPUS e não do slate: 40 de geração mais 60 de mistura, e as de mistura são servidas pela pista mista, que `RECIPES` não alcança. **Deixou de ser pendência do operador**: o número foi ratificado em 2026-08-12 (§ 3.3), e o que falta é trabalho de agente — crescer `RECIPES` até servir as 40. Os três slots de mistura por ilha **já existem** em `_island()`, chaveados pela operação e derivados de `MIX_OPERATIONS`, medido em 2026-08-17. Encolher a cota **não** reduz o número de ilhas: o piso é sobre fração (`1/n ≤ 0,05 + 0,02`), logo é livre de escala. É a guarda funcionando antes da cota, não defeito | unidade seguinte; a **cota de geração** que ela vai gastar continua sendo chave do operador |
| a **classe MISTA não tem slate, e a guarda que o cobra JÁ EXISTE**: `make_mixed.py` tem o seu `type=island_plan` no argparse, com as quatro pernas da pista de geração mais uma quinta — confere os três `mixingTemplates` da ilha contra `MIX_TEMPLATES` —, e a interseção dos dois conjuntos é **vazia**: o plano pede as **60** identidades `mix-<operação>-ilha-NN` e o arquivo serve **três** `mix_*_v1`, uma por banda de nudge, com os três escritores (`make_mixed.py`, `_agy`, `_codex`) pinando `mix_edit_v1`. Então a recusa é a guarda funcionando antes da cota, e o que falta é **material**. Sobra UMA dívida de MECANISMO: a banda foi consertada em 2026-08-18 — deixou de ser o literal `(0,05–0,7)`, que recusava os níveis 75 e 90, e passou a ser **derivada** de `MIX_LEVELS`, uma banda por nível do próprio nível ao ponto médio até o seguinte, regra que **reproduz** a única ratificada (v4 em [0,50–0,55]). Fica a identidade escrita por linha, que tem de vir da ilha × operação em vez de um nome fixo — e o docstring de `island_plan` diz que a ilha devolvida não é lida em ponto algum de `main`, então fiar a ilha até o escritor faz parte. A cota mista ainda não foi gasta, então a janela está aberta | unidade seguinte, antes de gerar a classe mista |
...
| o **recibo de pré-exposição não carrega identidade de bloco**, e é por isso que a amarração do corte fecha o processo e não o argumento. O que ficou **medido e fechado**: o corte abençoado antes da lease viaja como digesto até quem decide; a retomada o confere contra o recibo antes de reescrevê-lo; e uma invocação fresca sobre bloco que **aquele ledger** registra como gasto é recusada **antes** do write, ancorada em `assertHoldoutAvailable` — a mesma função que roda sob o lock, então as duas não podem discordar sobre o que "gasto" significa. O que fica **aberto**, com as duas sequências medidas: (i) uma corrida apontada a um `--ledger` **vazio** lê o bloco como não gasto e reescreve o recibo, e a retomada seguinte **completa** sobre o corte pós-exposição; (ii) sem malícia nenhuma, uma segunda `consume-holdout` legítima sobre **outro** bloco no mesmo `--work-dir` sobrescreve o recibo do primeiro, e a retomada dele passa a ser recusada para sempre. A causa é uma só: o recibo é chaveado **apenas** pelo `--work-dir` e não nomeia `consumptionId` nem campo algum do bloco. Fechar exige o recibo chaveado pela consumação — e a objeção que derrubou a alternativa anterior ("protege um arquivo com um arquivo") vale para a amarração do ramo de retomada, que é um JSON sem assinatura no diretório de trabalho | unidade própria, antes da Fase 5 — e **antes** de `consume-holdout` ser apertado |

...
| a matriz **nível × operação** e a **geometria da emenda** estão fixadas por **sonda e não por teste**: os Jaccard de 0,848–0,869 de v1 × inserção e o 0,774 da célula seguinte foram medidos com as funções de produção de `near_dupes.py` sobre pais sintéticos de 100 a 1.200 tokens, com tokens todos distintos — suposição declarada, porque texto real repete token —, e os 20 componentes de 500 da forma de três clusters foram medidos com `connected_components` sobre um plano emendado construído fora da árvore. A unidade que emendar `_island()` tem de prender os quatro casos, **inclusive os dois que racham** (cobertura só coletiva, e pai de uma paridade só): sem o caso vermelho, a guarda enuncia condição suficiente e não o critério | unidade que escrever `make_mixed_v3.py` e emendar `_island()` |

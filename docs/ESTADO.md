...
| `evaluatorDigest` da árvore | `d69f62bc4980f28546e60490b8e9c77ff4fc981278f44198ada0f0f4e62ab956` — 52 arquivos, recomputado pela função de produção e **lido por teste nomeado** (`digests.test.ts`, "is published in the ESTADO at the value the LIVE tree hashes to"), então este número não pode envelhecer em silêncio. Mover é barato enquanto `issuedAt` é nulo |
...
| dívida | vence |
|---|---|
| a **fila do cross-review de dez unidades está aberta em oito delas**: R1 pagou o `consolidado-c` e R2 o `consolidado-w1`, e os outros oito relatórios nunca foram processados — nem consertados, nem refutados, nem declarados. Medido em 2026-08-10, achado por achado contra o HEAD, com refutação adversarial dos bloqueantes: **23 bloqueantes confirmados** (9 mordem agora, as nove linhas abaixo; 14 mordem antes de publicar), **7 refutados** e **31 menores** ainda não refutados um a um. A lista itemizada, com o que caiu e por quê, está no registro (§ "A auditoria de 2026-08-10") | as nove abaixo, agora; as catorze, antes da Fase 6 |
| `validate` **sela e sai 0 sobre corpus que a auditoria marcou `blocked`**: `benchmark/commands/validate.ts:121-130` grava `source-readiness.json` e devolve `Dataset sealed: …` sem ler `readiness.status`, e a face que recusa (`assertMaterialBatchesResolve`, `benchmark/schema.ts:3851`) segue sem chamador de produção com docstring afirmando o contrário. Nenhuma calibração congela sobre corpus bloqueado (`candidate-preflight.ts:294`, `commands/fit.ts:291`); o dano é o comando de selagem publicar sucesso | antes de o operador rodar `validate` na Fase 3 |
| **`groups: {}` atravessa parse, evento e atestado do ledger**: `cluster-exposure-ledger.ts:534-539` só exige objeto não nulo, `buildEventRecords` (:1892-1893) itera as entradas e sai com `groupDigests` vazio, `validateEventShape` confere presença de chave e não completude, e `exposureInputsFromRecords` (:2505) — o único que derivaria os eixos dos registros — não tem chamador de produção. Exposição registrada sem eixo é barreira de cluster que não barra | antes do congelamento do split (Fase 3, item 3) |
| **`extractionRun` é o nome do arquivo de pool, não a execução da extração**: `assemble_corpus.py:2676` estampa `extraction_{fname}` por `setdefault`, então a recusa `MissingExtractionRun` (:1361-1366) **nunca morde no caminho de pool** e o valor inventado chega ao registro selado como `known`; os extratores não emitem o eixo | Fase 3, item 2 |
| **a guarda de viabilidade recusa o corpo que os próprios construtores montam**: `assert_components_can_fill_five_partitions` é a primeira instrução de `assign_partitions` (`assemble_corpus.py:2591`) e a fração é **por classe** (:2288-2318), enquanto `promptTemplate` (:1104) e `generatorVersion` (:1106) unem a classe gerada num componente que lê 1,0000 — `UnsplittableCorpus` aborta antes de qualquer carimbo. Tirar só `generatorVersion` deixa o maior em 54,79 %, acima de 45 % + 2 pp: continua recusado, e `promptTemplate` não estava declarado em dívida nenhuma | Fase 3, item 2, antes da primeira montagem com classe gerada |
| **`requiredHumanSourceTypes` exige presença, não pertença**: `dataset-manifest.ts:915-921` reprova só quando a contagem da célula é zero, `:894-896` não conta a ausência e `humanSourceType` é opcional no esquema (`schema.ts:2245-2250`) — linha humana sem célula passa o gate da moldura | antes do selo da Fase 3 |
| **o manifesto de fontes promete não carregar URL e o produtor grava uma**: `build_governance.ts:75-79` põe `https://dumps.wikimedia.org/…` em `evidence`, contra `source-manifest.ts:23-31` ("carries NO source URL, name, handle or raw consent receipt") e o runbook:427-428. Nenhuma guarda confere | antes de o manifesto entrar no pacote da Fase 6 |
| **o gate de composição não deixa recibo**: `auditReleaseComposition` tem um sítio só (`commands/split.ts:169`, dentro de `scientificUse === "release"`), o veredito só alimenta a mensagem de erro, e nenhuma das 14 chaves de `SEALED_ARTIFACT_KEYS` (`split-artifact.ts:103-118`) o carrega — passar não deixa prova | Fase 3, item 3 |
| **o piso barato compara as duas AUCs contra o mesmo vetor humano inteiro**: `baseline_tfidf.py:717-726` monta `reserved` (108) e `core` (145) contra as mesmas 253 humanas, sem `class_weight`, e `read_ood_easiness` lê uma contra a outra — o pareamento OOD vale para a união, não dentro de cada família | Fase 3, item 2, antes de o critério da reservada decidir |
| **a junção parcial da sonda passa silenciosa**: `baseline_tfidf.py:740-747` só recusa população **vazia**, então 20 de 253 humanas casadas rodam a AUC enquanto `block["rows"]` (:707-716), montado antes de o arquivo de escores ser lido, continua publicando 108/145/253 do pool | Fase 3, item 2 |
...
| `fit-report.json` continua lido com `as FitReport` em `publish-evidence.ts`, e só o **bloco do corte** é parseado (pelo parser fechado do artefato, com código próprio `FIT_REPORT_CUT_MALFORMED`). O resto do arquivo é cast, mesmo padrão que o terceiro achado condenou no artefato do limiar; o que foi fechado é o único sítio onde o cast era seguido de dereference | unidade que der um parser a `fit-report.json` |

...
| suíte | 172 arquivos / 3.114 testes (vitest) + 740 testes e 742 subtests (pytest, lab). Verde em rodada limpa e SOZINHA; com uma segunda corrida de vitest concorrente, dois a quatro arquivos de caminho selado batem no timeout de 20 s — dívida de § 7, não de política |
| dos quais, o avaliador | 2.477 — 1.737 em 46 arquivos de `benchmark/tests`, 740 no lab |
| typecheck | limpo |
...
propriedade da lane**: `configurable` é verdade exactamente quando a fonte do REGISTRO é `flag`, e
a mesma lane escreve as duas formas.
...
| a **forma da linha de lane** admite as seis lanes, e o que fica em aberto é material e não forma. O canal separa duas coisas antes colapsadas — `harnessVersion` aplica-se em todo canal que não seja `api`, os knobs de amostragem pertencem a `api` e a `local-runtime`, e `ollama` é a única lane em que as duas valem. `decodingConfigurable` é conferido contra o canal por IGUALDADE, nas duas direções: negar os knobs de um canal que os expõe descarta o `seed`, que é o único campo que torna uma geração reproduzível. A configurabilidade de effort saiu da linha de lane e é derivada do REGISTRO (`configurable` ⇔ fonte `flag`), o que torna a fonte `flag` do agy gravável e não cria arm inabitável; `effortLevels` passou a ser a escala completa (`codex` com seis níveis), porque o campo são os níveis da escala e uma lista curta é falsidade; e `not-supported` é fato do registro — a lane, o modelo ou a forma de invocação não tinham o controle —, o que torna escrevíveis as 122 linhas da sessão interativa. Uma lane sem escala oferece `not-supported` e **nada mais**, por igualdade: qualquer outra fonte carrega nível, e nível precisa de escala. Bateria de sete mutações, cada uma vermelha no teste nomeado | **feita** em 2026-08-21 |

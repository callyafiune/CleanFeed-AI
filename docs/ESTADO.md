...
| suíte | 172 arquivos / 3.097 testes (vitest) + 709 testes e 505 subtests (pytest, lab). Verde em rodada limpa e SOZINHA; com uma segunda corrida de vitest concorrente, dois a quatro arquivos de caminho selado batem no timeout de 20 s — dívida de § 7, não de política |
| dos quais, o avaliador | 2.429 — 1.720 em 46 arquivos de `benchmark/tests`, 709 no lab |
| typecheck | limpo |
...
| a **classe MISTA não tem slate, e a guarda que o cobra JÁ EXISTE**: `make_mixed.py` tem o seu `type=island_plan` no argparse, com as quatro pernas da pista de geração mais uma quinta — confere os três `mixingTemplates` da ilha contra `MIX_TEMPLATES` —, e a interseção dos dois conjuntos é **vazia**: o plano pede as **60** identidades `mix-<operação>-ilha-NN` e o arquivo serve **três** `mix_*_v1`, uma por banda de nudge, com os três escritores (`make_mixed.py`, `_agy`, `_codex`) pinando `mix_edit_v1`. Então a recusa é a guarda funcionando antes da cota, e o que falta é **material**. Sobram duas dívidas de MECANISMO, as duas pequenas: a banda legada `MIXED_BAND (0,05–0,7)` **recusaria os níveis 75 e 90** da curva ratificada, e a identidade escrita por linha tem de vir da ilha × operação em vez de um nome fixo. A cota mista ainda não foi gasta, então a janela está aberta | unidade seguinte, antes de gerar a classe mista |

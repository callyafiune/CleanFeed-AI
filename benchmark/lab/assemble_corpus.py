...
# ONE ENTRY, and the second ratified lineage is absent BY MEASUREMENT rather than by
# choice: the reserve is two families of 450 lines each (`qwen2.5:7b` and
# `llama3:8b-instruct-q4_K_M`, both Q4_K_M so lineage is the only variable), and the
# llama has no line on disk yet. Declaring it here would be refused by
# `assert_slate_roles_are_consistent` — a role over a family the pools never deliver is
# coverage of nothing, which is the defect that guard exists against. The direction of
# the gap is fail-closed: when the llama lines exist and no role names them, assembly
# HALTS on `UndeclaredGeneratorFamily` rather than training on them, so the entry cannot
# be forgotten into silence.
